...
def runFermiTools(Name,RA,DEC,minEnergy,maxEnergy,SCFile,radius,binsz,TSTART,TSTOP,Evfile,bins,zmax,evclass,evtype,TSul,NMtol,lc_bin_num,runMRM):
    ...
    ########################################################
    #Run likelihood a second time for null hypothesis
    ex.run(Name,P='0',I='0',S='0')
    #Output is Name_null.xml and sets PowerLaw to fixed values from the output values
    obs2 = UnbinnedObs(Name + '_gtmktime' + str(lc_bin_num) + '_lc.fits', SCFile,expMap= Name + '_expMap' + str(lc_bin_num) + '_lc.fits',expCube= Name + '_ltcube' + str(lc_bin_num) + '_lc.fits', irfs='P8R2_SOURCE_V6')
    analysis2 = UnbinnedAnalysis(obs2,Name + '_null.xml', optimizer='NewMinuit')
    likeObj2 = pyLike.NewMinuit(analysis2.logLike)
    LIKE2 = analysis2.fit(verbosity=0,covar=True,optObject=likeObj2)
    ########################################################
    ...
    f = open(str(Name) + "_lc_output.txt", "a")
    f.write(str(Flux) + ',' + str(Ferr) + ',' + str(ef) + ',' + str(ef_err) + ',' + str(limit) + ','+ str(analysis.Ts(Name)) + ',' + str(UL) + ',' + str(TMID) + ',' + str(exp) + ',' + str(LIKE) + ',' + str(LIKE2) + '\n')
    f.close()
